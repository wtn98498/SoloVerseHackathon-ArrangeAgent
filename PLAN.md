...
1. 用户选择风格，或直接开始玩。
2. 用户在一个乐器上玩 5-10 秒。
3. App 把用户输入记录成轻型 MIDI clip / seed pattern。
4. Agent 把这段 MIDI seed 扩成一段 8 小节乐队 loop。
5. 用户可以继续让 Agent 改编，例如“更有能量”或“更柔和”。
...
说明：MVP 不做完整 DAW 级 MIDI 编辑器，但必须有轻型 MIDI 能力：
8 小节 clip、note/drum-hit 事件、piano-roll 风格可视化、播放指针和 Agent
补全/能量变化。按键发音只是录入方式，不是产品核心。

钢琴卷帘只做三个八度：`C2` 到 `B4`。所有生成、校验、可视化和本地 MIDI
编辑都必须守住这个范围；不要扩展成全键盘或完整 MIDI 编辑器。

### 当前产品决策

底层保留 lightweight MIDI-like JSON，不迁移到第三方 MIDI/DAW 引擎。

开源项目只做参考，不直接接入大型子系统：

- `two-moons` / MoaRoll：主要参考钢琴卷帘的视觉、键盘、网格和 playhead
  体验；不接入它的 MobX store、秒级 timing model 或 MIDI 导出系统。
- `openDAW`：只参考现代 Web DAW 布局气质；MVP 不接入，避免 scope 和 license
  风险。
- `GridSound DAW`：只参考浏览器 DAW 概念；MVP 不接入。
- Tone.js examples：继续作为播放调度参考。

详细决策见 `docs/agent-design.md`。
...
- 编曲格式：lightweight MIDI JSON clip model
...
本项目要做的是小型 AI 音乐草稿工具，不重复造完整 DAW，也不把大型开源项目搬进来。
卷帘界面可以“像 MIDI”，但产品卖点是 Agent 把一个小 seed 扩成可听、可见、可改的
8 小节乐队 loop。
...
Agent 必须输出结构化 JSON，前端验证后再应用。不要让 Agent 输出自由文本再靠字符串解析改音乐；也不要只触发音频，必须改变可视化 MIDI clip 数据。
